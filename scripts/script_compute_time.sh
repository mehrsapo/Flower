...
## D FLOW
method=d_flow
python main.py --opts solve_inverse_problem True dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} lmbda 0.001 alpha 0.1 max_batch ${batch_size_ip} batch_size_ip 2 max_iter 7 compute_memory True compute_time True save_results False device cuda:0