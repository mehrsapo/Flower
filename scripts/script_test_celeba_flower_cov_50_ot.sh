...
# FLOWER 
problem=denoising
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} num_samples 50 max_batch ${max_batch} batch_size_ip ${batch_size_ip} steps 100 device cuda:3
problem=inpainting
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} num_samples 50 max_batch ${max_batch} batch_size_ip ${batch_size_ip} steps 100 device cuda:3
