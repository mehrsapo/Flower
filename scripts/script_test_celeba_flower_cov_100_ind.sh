...
# FLOWER 
problem=denoising
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} num_samples 100 max_batch ${max_batch} batch_size_ip ${batch_size_ip} steps 100 device cuda:0
problem=inpainting
python main.py --opts dataset ${dataset} eval_split ${eval_split} model ${model} problem ${problem} method ${method} num_samples 100 max_batch ${max_batch} batch_size_ip ${batch_size_ip} steps 100 device cuda:0
